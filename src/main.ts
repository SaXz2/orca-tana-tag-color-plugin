...
async function getAllPanelBlockIds() {
  ...
  // 读取所有面板的容器块元素
  readAllPanelsContainerBlocks(viewPanels);
  
  return blockIds;
}

/**
 * 读取所有面板中的容器块 data-id，并筛选出带标签的块
 */
function readAllPanelsContainerBlocks(viewPanels: any[]) {
  for (const panel of viewPanels) {
    ...
    containerElements.forEach((element) => {
      ...
      if (reprMainElement) {
        ...
        if (tagsElement) {
          // 如果有标签，记录这个容器块的 data-id
          const dataId = element.getAttribute('data-id');
          if (dataId) {
            taggedBlockIds.push(dataId);
          }
        }
      }
    });
    ...
    if (taggedBlockIds.length > 0) {
      console.log(`当前面板 [${panelId}] 的带标签容器块 data-id:`, taggedBlockIds);
    }
  }
}
